 ...
 * Env vars required: RESEND_API_KEY, DIGEST_EMAIL
...
export async function POST(req: NextRequest) {
  try {
    // ── Validate env vars first — return clear errors ──────────────────────
    ...
    if (!to) {
      return NextResponse.json(
        { error: 'No recipient email — pass { "email": "you@example.com" } in the request body, or set DIGEST_EMAIL in your .env file.' },
        { status: 400 }
      );
    }
    ...
    const html = buildDigestHtml({
      today, allClients, pending, overdueTasks,
      expiringIn30, unreadReminders, totalPaid,
    });

    // ── Use onboarding@resend.dev as fallback sender ───────────────────────
    // This works on ALL Resend accounts without domain verification.
    // To use your own domain, set RESEND_FROM_EMAIL=alerts@yourdomain.com
    ...
    if (!resendRes.ok) {
      ...
      // Surface helpful Resend-specific error messages
      const resendMsg = (parsed?.message as string) || resendBody;
      let friendly = resendMsg;

      if (resendMsg.includes('domain is not verified')) {
        friendly = `The "from" address "${fromAddress}" uses a domain not verified in Resend. Either verify your domain in the Resend dashboard, or leave RESEND_FROM_EMAIL unset to use onboarding@resend.dev.`;
      } else if (resendMsg.includes('Invalid API key')) {
        friendly = 'Your RESEND_API_KEY is invalid. Check the key in your Resend dashboard.';
      } else if (resendMsg.includes('not allowed to send') || resendMsg.includes('testing emails')) {
        friendly = `Resend free tier only allows sending to your verified email address. The recipient "${to}" must match your Resend account email, or upgrade to a paid plan to send to anyone.`;
      }

      rollbar.error('[email-digest] Resend rejected', { status: resendRes.status, body: resendMsg });
      return NextResponse.json(
    ...
    rollbar.error('[POST /api/email-digest]', error);
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    resendKey:   hasKey  ? 'set ✓' : '❌ MISSING — set RESEND_API_KEY in .env',
    digestEmail: hasEmail
      ? `set ✓ (${process.env.DIGEST_EMAIL})`
      : '❌ MISSING — set DIGEST_EMAIL in .env',
...
<style>
  ...
  .stat .num{font-size:26px;font-weight:900;color:#1a1a2e}
  .stat .lbl{font-size:10px;color:#9b96b0;font-weight:700;text-transform:uppercase;margin-top:4px;letter-spacing:.06em}
  ...
  .il strong{display:block;font-weight:700;color:#1a1a2e;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
<div class="wrap">
  ...
  <div class="body">
    ...
    <div class="section">
      ...
      ${pending.slice(0, 8).map(p => {
        ...
        return `<div class="item">
          <div class="il"><strong>${p.invoiceNumber || 'Invoice'}</strong><span>${c?.businessName ?? 'Unknown Client'}</span></div>
          <span class="badge ${age > 30 ? 'badge-red' : 'badge-amber'}">GHS ${p.amount.toLocaleString()} · ${age}d old</span>
        </div>`;
      }).join('')}
    ...
    <div class="section">
      <h2>Domains Expiring in 30 Days (${expiringIn30.length})</h2>
      ${expiringIn30.slice(0, 8).map(w => {
        ...
        return `<div class="item">
          <div class="il"><strong>${w.domainName}</strong><span>${c?.businessName ?? ''}</span></div>
    ...
    <div class="section">
      ...
      ${overdueTasks.slice(0, 6).map(t => {
        ...
        return `<div class="item">
          <div class="il"><strong>${t.description.slice(0, 55)}${t.description.length > 55 ? '…' : ''}</strong><span>${c?.businessName ?? ''}</span></div>
    ...
    <div class="section">
      ...
      ${unreadReminders.slice(0, 6).map(r => `
        <div class="item">
          <div class="il"><strong>${r.title}</strong><span>${r.type}</span></div>
          <span class="badge badge-blue">${r.date}</span>
        </div>`).join('')}
    </div>` : ''}

  </div>
  <div class="ftr">TGNE Agency · Accra, Ghana · Automated Digest · Do not reply to this email</div>
</div>
</body></html>`;
}
