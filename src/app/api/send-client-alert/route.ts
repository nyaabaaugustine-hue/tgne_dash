...
<div style="max-width:620px;margin:32px auto;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
  ...
  <div style="padding:28px 40px 0;">
    ...
    <p style="font-size:14px;color:#64748b;line-height:1.7;margin:0;">This is a friendly reminder from TGNE regarding upcoming items for <strong>${businessName}</strong>.</p>
  </div>
  ...
  <div style="margin:0 40px 32px;background:#f8faff;border:1px solid #e0e7ff;border-radius:14px;padding:22px;text-align:center;">
    ...
    <a href="mailto:info@tgne.co" style="background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;font-weight:800;font-size:13px;padding:12px 28px;border-radius:10px;text-decoration:none;">Contact Your Account Team</a>
  </div>
...
async function sendViaResend(to: string, subject: string, html: string): Promise<{ ok: boolean; error?: string }> {
  ...
  if (msg.includes('testing emails') || msg.includes('not allowed to send') || msg.includes('You can only send')) {
    msg = 'Resend free tier only allows sending to your own verified email. Upgrade Resend or use an address matching your Resend account.';
  } else if (msg.includes('domain is not verified')) {
    msg = `From-address domain not verified in Resend. Leave RESEND_FROM_EMAIL unset to use onboarding@resend.dev.`;
  }
...
async function sendViaBrevo(to: string, toName: string, subject: string, html: string): Promise<{ ok: boolean; error?: string }> {
  ...
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({ sender: { name: senderName, email: senderEmail }, to: [{ email: to, name: toName }], subject, htmlContent: html }),
  });
  if (res.ok) return { ok: true };
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!client.email) return NextResponse.json({ error: `Client "${client.businessName}" has no email address on file.` }, { status: 400 });
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(client.email)) return NextResponse.json({ error: `Client email "${client.email}" appears invalid.` }, { status: 400 });
    ...
    if (alerts.length === 0) {
      return NextResponse.json({ sent: false, message: `No items found for "${client.businessName}" within ${daysWindow} days.` });
    }
    ...
    let result   = await sendViaResend(client.email, subject, html);
    let provider = 'Resend';
    ...
    if (!result.ok) {
      ...
      if (!result.ok) {
        return NextResponse.json({
          sent: false, error: 'Email could not be sent via Resend or Brevo.',
          resend: resendError, brevo: result.error,
          hint: 'Set RESEND_API_KEY or BREVO_API_KEY + BREVO_SENDER_EMAIL in your .env file.',
        }, { status: 502 });
      }
    }

    return NextResponse.json({ sent: true, to: client.email, alertCount: alerts.length, provider });
